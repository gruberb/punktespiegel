...
Historische Transfermarkt-Kaderseiten liefern einzelne stabile Biografiefelder nicht immer erneut aus. Der Backfill übernimmt dann Geburtsdatum, Nationalität, Größe und Fuß aus dem aktuellen Snapshot desselben Transfermarkt-Spielers. Eintrittsdatum und vorheriger Verein werden nur ergänzt, wenn der Spieler im aktuellen und historischen Snapshot beim selben Transfermarkt-Verein steht. Das Alter wird am Ende der gewählten Saison berechnet; aktuelle Trainer werden bewusst nicht rückwirkend als historische Trainer ausgegeben. Karriereartefakte enthalten neben Vereinssummen auch Zeilen je Saison und Wettbewerb.
...
`.github/workflows/pages.yml` läuft mit GitHubs zeitzonenbewusstem Scheduler täglich um **12:15 Uhr deutscher Zeit**. Ein zweiter Lauf um **14:45 Uhr deutscher Zeit** dient als Absicherung, falls der erste Zeitplan unter hoher GitHub-Last verspätet oder gar nicht ausgelöst wird. Zusätzlich läuft der Workflow bei jedem Push auf `main`. Die täglichen Läufe decken sowohl die üblichen Montagswertungen als auch Donnerstagswertungen nach englischen Wochen ab. Sie lassen sich unter **Actions → Daten aktualisieren und Pages bauen → Run workflow** wiederholen.
